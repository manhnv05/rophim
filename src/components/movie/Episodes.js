...
const MovieEpisodes = ({movie, page}) => {
    ...
    useEffect(() => {
        const getSeasons = async () => {
            if (!isLoading) {
                setIsLoading(true)
                const result = await MovieApi.seasons(movie._id)
    ...
    useEffect(() => {
        if (curSeason && curVersion) {
            ...
            setVersions(_types)
            if (_types.indexOf(parseInt(curVersion)) < 0) {
                dispatch(setCurVersion(_types[0]))
            }

            const episodes = curSeason?.episodes.filter(el => el.versions.filter(el2 => el2.type == curVersion).length > 0)
                            ...
                            return (
                                <Link onClick={(e) => handleEpisodeClick(e, ep)} key={`ep-${ep._id}`}
                                    ...
                                    <div className="v-thumbnail h-thumbnail">
                                        ...
                                        <img src={episodeThumbnail(ep.image_path)} alt={`Tập ${ep.episode_number}`}
                                             loading="lazy"/>
                                    </div>
                                    <div className="info">
                                        <div className="play-button">
                                        ...
                                        <div className="ep-sort flex-shrink-0">Tập {ep.episode_number}</div>
                                        <div className="media-title">{ep.name}</div>
                                    </div>
                                </Link>
                            )
                        })}
